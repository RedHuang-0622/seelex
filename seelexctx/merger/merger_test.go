...
func TestMergeBack_NilParent(t *testing.T) {
	if err := NewMerger().MergeBack(nil, &snapshot.ContextSnapshot{}); err == nil {
		t.Fatal("expected error")
	}
}

func TestMergeBack_NilChild(t *testing.T) {
	if err := NewMerger().MergeBack(&snapshot.ContextSnapshot{}, nil); err == nil {
		t.Fatal("expected error")
	}
}
...
func TestMergeBack_FindingsAppend(t *testing.T) {
	...
	if err := NewMerger().MergeBack(p, c); err != nil {
		t.Fatal(err)
	}
	if len(p.Findings) != 2 {
		t.Fatalf("got %d", len(p.Findings))
	}
}
...
func TestMergeBack_DecisionsAppend(t *testing.T) {
	...
	if len(p.Decisions) != 2 {
		t.Fatalf("got %d", len(p.Decisions))
	}
}
...
func TestMergeBack_ProgressReplace(t *testing.T) {
	...
	if p.Progress != "new" {
		t.Fatalf("got %q", p.Progress)
	}
}

func TestMergeBack_ProgressKeep(t *testing.T) {
	p := &snapshot.ContextSnapshot{Progress: "keep"}
	if err := NewMerger().MergeBack(p, &snapshot.ContextSnapshot{}); err != nil {
		t.Fatal(err)
	}
	if p.Progress != "keep" {
		t.Fatalf("got %q", p.Progress)
...
func TestMergeBack_PendingReplace(t *testing.T) {
	...
	if len(p.PendingWork) != 2 {
		t.Fatalf("got %d", len(p.PendingWork))
	}
}

func TestMergeBack_TokenSum(t *testing.T) {
	p := &snapshot.ContextSnapshot{TokenEstimate: 100}
	if err := NewMerger().MergeBack(p, &snapshot.ContextSnapshot{TokenEstimate: 50}); err != nil {
		t.Fatal(err)
	}
	if p.TokenEstimate != 150 {
		t.Fatalf("got %d", p.TokenEstimate)
	}
}

func TestMergeBack_MessageCountSum(t *testing.T) {
	p := &snapshot.ContextSnapshot{MessageCount: 5}
	if err := NewMerger().MergeBack(p, &snapshot.ContextSnapshot{MessageCount: 3}); err != nil {
		t.Fatal(err)
	}
	if p.MessageCount != 8 {
		t.Fatalf("got %d", p.MessageCount)
	}
}

func TestMergeBack_EscapeReplace(t *testing.T) {
	p := &snapshot.ContextSnapshot{Escape: &snapshot.EscapeInfo{Reason: "old"}}
	if err := NewMerger().MergeBack(p, &snapshot.ContextSnapshot{Escape: &snapshot.EscapeInfo{Reason: "new"}}); err != nil {
		t.Fatal(err)
	}
	if p.Escape.Reason != "new" {
		t.Fatalf("got %q", p.Escape.Reason)
	}
}

func TestMergeBack_ConstraintsDedup(t *testing.T) {
	p := &snapshot.ContextSnapshot{Constraints: []string{"c1", "c2"}}
	if err := NewMerger().MergeBack(p, &snapshot.ContextSnapshot{Constraints: []string{"c2", "c3"}}); err != nil {
		t.Fatal(err)
	}
	if len(p.Constraints) != 3 {
		t.Fatalf("got %d: %v", len(p.Constraints), p.Constraints)
	}
}
...
func TestMergeBack_CopyOnWrite(t *testing.T) {
	...
	if err := NewMerger().MergeBack(p, &snapshot.ContextSnapshot{Decisions: []snapshot.Decision{{What: "c", Why: "w"}}}); err != nil {
		t.Fatal(err)
	}
	if len(orig) != 1 {
		t.Fatal("original should be unmodified")
	}
}
...
func TestMergeBack_Alternatives(t *testing.T) {
	...
	if len(p.Decisions[0].Alternatives) != 2 {
		t.Fatal("alternatives should be preserved")
	}
	if len(p.Decisions[1].Alternatives) != 1 {
