...
func TestCompact_FullBudget(t *testing.T) {
	...
	if r == snap { t.Fatal("should be copy") }
	if len(r.Findings) != 2 { t.Fatalf("got %d", len(r.Findings)) }
}
...
func TestCompact_Summary(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if len(r.Decisions) != 1 { t.Fatalf("expected 1 summary decision, got %d", len(r.Decisions)) }
	if len(r.Findings) != 1 { t.Fatalf("expected 1 summary finding, got %d", len(r.Findings)) }
}
...
func TestCompact_Minimal(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if len(r.Findings) != 0 { t.Fatal("expected 0 findings in minimal") }
	if len(r.Decisions) != 0 { t.Fatal("expected 0 decisions in minimal") }
}

func TestCompact_NegativeBudget(t *testing.T) {
	r, err := NewCompactor().Compact(&snapshot.ContextSnapshot{Goal: "t"}, -1)
	if err != nil { t.Fatal(err) }
	if r.Goal != "t" { t.Fatal("expected goal preserved") }
}
...
func TestEstimateTokens(t *testing.T) {
	...
	if n <= 0 { t.Fatalf("got %d", n) }
}

func TestTruncateForToken(t *testing.T) {
	if r := truncateForToken("short", 100); r != "short" { t.Fatalf("got %q", r) }
	r := truncateForToken(strings.Repeat("a", 500), 5)
	if len(r) >= 500 { t.Fatal("expected truncation") }
}
...
func TestCompact_SummaryToolCalls(t *testing.T) {
	...
	r, err := NewCompactor().Compact(snap, 250)
	if err != nil { t.Fatal(err) }
	if len(r.Decisions) != 1 { t.Fatalf("expected 1, got %d", len(r.Decisions)) }
}
