...
> **⚠️ 本文已过时**
>
> 本文描述的是已删除的 `commandstack/` 包。当前所有 MCP 调用的追溯由通用 `mcpstack/` 中间件提供（见 [`../arch/design-decisions-mcp-storage.md`](../arch/design-decisions-mcp-storage.md)）。

> 版本: v1.0  
> 创建日期: 2025-07-18  
> 状态: ❌ 已废弃（`commandstack/` → `mcpstack/`）  
