...
| **S27** | R2/R4 端口/DTO 收口：把当前 core 内部 `roleSessionPort` 与 headless 的 sessionstore 透传形状迁到 `application/contract` 纯 DTO（`RoleSessionPort`/`RoleDraftPort`/`SequencerPort`/`SchedulePort`），避免 GUI 直接依赖存储类型 | §8.3、应用层接线提示词 §R2/R4 | `application/contract/ports.go`、`application/contract/dto`、`application/core/role_session.go`、`internal/adapters`、`gui/headless.go` | 现有 S26 headless 真实 API 冒烟改用 DTO 后仍通过。**进度（S28）**：新增的 `team.*` 装配面已全程纯 DTO（`dto.TeamSpec/TeamView/TeamRegistry` + 适配器映射，gui 不 import sessionstore 类型）；`role.*`/`schedule.*` 仍是 sessionstore 类型透传，**未收口** |
