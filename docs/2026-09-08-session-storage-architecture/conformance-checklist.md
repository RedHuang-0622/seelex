...
| S25 R2/R3/R4 基建（goal 角色会话 / role draft+sequencer+floor / 物化缓存接口） | `go test ./sessionstore -run 'TestRoleSession\|TestGoalStackRecordsRoleSessionAnchor\|TestRoleDraftSyncOrderFloorAndIdempotency\|TestLifecycleOrderAndRoleRefSurviveDraftCommit\|TestMaterialCacheInvalidation\|TestScheduleEventsAreRecorded' -count=1` → **ok**；`go vet ./sessionstore` 干净 | — | 存储侧新增：角色会话子树 `goal_<hash>/`/`role_<hash>/`（复用 storeEngine）、goal 栈条目 `role_name/role_session_id`、`Event` 角色字段、`messageHead.Floor`、role draft append/read/sync 幂等与同步即删、`lifecycle.order_policy/order_roles/join_seq_id/compact_ref`、`schedule.*` EVENT、`MaterialCache` 接口与失效键。应用层 actor/EVENT 生产者未接线 |
| S26 群聊基线接入相关链路 + headless 真实 API/pprof | `go test ./application/... ./sessionstore ./internal/adapters ./gui -count=1` → **ok**；`go build ./...` 与 `go build -tags "gui,desktop,production" ./...` → **ok**；`go vet ./sessionstore ./internal/adapters ./application/core/... ./gui` → **ok**；`go test -race ./sessionstore -run 'TestRoleSession\|...'` / `-race ./internal/adapters` / `-race ./gui -run TestHeadlessRoleRPC` / `-race ./application/core/task_context` → **ok** | `go build -race -tags pprof -o tmp/headless-smoke/seelex-pprof-role-race.exe .`；`$env:SMOKE_ROLE_LIVE='1'; $env:SMOKE_ROLE_LIVE_PPROF='1'; $env:GORACE='halt_on_error=1'; go test ./gui -run TestRealAPIRoleSessionLiveProbe -v -count=1` → **PASS**，无 data race；报告落 `tmp/headless-smoke/reports/role-mutex-*.txt`、`role-block-*.txt` 与 `fork-live-goroutine-role-live-*.txt` | **链路同步**：`model.TranscriptEvent` + `internal/adapters` 保留 `role_name/role_session_id/round_id/unit_seq`；`task_context` 统一给 user/main/system 行盖角色归属与 round/unit 排序键；`sessionstore` 增加角色 wire（main compact_ref + `seq > max(join_seq_id, applied_seq)` + 自身 pending draft）与 `RoleSnapshot` 观察面；`gui/headless` 暴露 `role.*`/`schedule.*` RPC。**设计偏差观察**：首轮真实 API 冒烟发现 user/main 行缺 `role_name`（先 2 行、后 1 行），已在 `task_context` 生产者收口；重跑 `UnassignedRoleRows=0` 且 `DesignWarnings` 为空。**provider 实验**：`TestCustomRoleLiveProbe` 将 provider 历史 role 设为 `tl`，日志确认已原样发送，端点以 `unknown variant tl, expected system/user/assistant/tool/latest_reminder` 拒绝；因此 provider role 只能标准集，A2A 角色名必须留在 `role_name` metadata。**pprof 结论**：mutex profile 无 role draft/message/floor 链路热点；block profile 主要是 actor `select` 与 LLM stream 等预期阻塞；race-enabled 真实进程未报数据竞争 |
| S28 AgentTeam 工厂 + subagent tool-call 恢复 + provider system 收口 | `go test ./application/... ./sessionstore ./internal/adapters ./gui -count=1` → **ok**；`go test ./seelebridge/... ./session/... ./workspace/... -count=1`（非沙箱）→ **ok**；`go build ./...` 与 `go build -tags "gui,desktop,production" ./...` → **ok** | AgentTeam：`go build -tags pprof -o tmp/headless-smoke/seelex-pprof-team.exe .`；`SMOKE_TEAM_LIVE=1 SMOKE_TEAM_LIVE_PPROF=1 go test ./gui -run TestRealAPIAgentTeamLiveProbe` → **PASS**。subagent：`go build -race -tags pprof -o tmp/headless-smoke/seelex-pprof-subagent-race.exe .`；`SMOKE_SUBAGENT_LIVE=1 SMOKE_SUBAGENT_LIVE_PPROF=1 GORACE=halt_on_error=1 go test ./gui -run TestRealAPISubagentResumeLiveProbe` → **PASS**（真实中断 → 冷启动 → system 恢复说明 → 同键续跑 → 收敛，无 DATA RACE）。provider 位置：`SEELEX_LIVE_SMOKE=1 go test ./seelebridge -run 'TestCustomRoleLiveProbe\|TestSystemPositionLiveProbe'` 记录拒绝自定义 role、接受历史中部 system | **AgentTeam**：`application/core/agentteam`（RoleSpec/TeamSpec/Normalize/Factory/Registry）、`sessionstore/team_registry.go`、`team.*` headless RPC、`goal-a2a/review-team/research-team` preset；AT1/AT8 由真实 headless 冒烟验证，subagent 不进成员表、定时 agent 不入 order_roles。**subagent 恢复**：新增 `application/core/resume` 七步模板（定位→判定→补历史→重建现场→system 注入→同键重跑→收敛）与 `seelebridge/runtime_subagent_resume.go`；`subagent.list/fork/recover/resume` headless 暴露；真实探针用诊断 hold 确保 active 可观测，硬中断后清 stale lock 冷恢复续跑成功。**provider role**：task/goal/plan/subagent internal 状态材料在 provider 边界统一映射为 `system`，真实用户输入保持 `user`，激活技能正文保留 internal user 轮次以维持稳定前缀缓存；`TestSystemPositionLiveProbe` 证明历史中部 system 可用 |

| S26 R1 多后端实现收口（方案 A） | `go test ./sessionstore -count=1 -timeout=240s` → **ok 56.1 s**；`go test -race ./sessionstore -count=1 -timeout=400s` → **ok 44.0 s，无竞争报告**；`go test ./application/... ./internal/... ./seelebridge/... -count=1` 全绿；`go build ./...` 与 `go build -tags "gui,desktop,production" ./...` ok；`go vet ./sessionstore` 干净 | `go test ./sessionstore -run TestStackChannelLockAttribution -mutexprofile=<temp>.pb` → ok 5.9 s | 删除 `sqlRepository`/`redisRepository`、SQL/Redis 栈后端、manifest/generation/shard 实现与 sqlite 矩阵半边；保留 `Repository`/`stackJournal` 与 JSON v8；`Normalize`/`Open` 对三退役枚举返回 `ErrBackendRetired`（`TestRetiredBackendsReturnExplicitError`）；README 标注“后端实现已收口，新后端待按接口实现” |

| S28 AgentTeam 角色工厂 + 角色注册表 + `team.*` 装配面（工作流 B） | `go test ./application/core/agentteam ./sessionstore ./gui -count=1` → **ok**；全量 `go test ./application/... ./sessionstore ./internal/adapters ./gui -count=1` → **ok**（90.3 s）；`go build ./...` 与 `go build -tags "gui,desktop,production" ./...` → **ok**；`go vet ./gui ./application/core/agentteam ./sessionstore ./internal/adapters` → **ok**；`go test -race ./application/core/agentteam -count=1` → **ok，无竞争**；`go test ./e2e/ -run 'TestRepositoryModulesHaveReadmes\|TestRepositoryModuleReadmeLinks\|TestRepositoryAgentDocumentationRules'` → **ok** | `go build -tags pprof -o tmp/headless-smoke/seelex-pprof-team.exe .` 与 `go build -race -tags pprof -o tmp/headless-smoke/seelex-pprof-team-race.exe .`；`$env:SMOKE_TEAM_LIVE='1'; $env:SMOKE_TEAM_LIVE_PPROF='1'; GORACE=halt_on_error=1; go test ./gui -run TestRealAPIAgentTeamLiveProbe -v -count=1` → **PASS（含 race 目标）**，报告 `tmp/headless-smoke/reports/team-live-20260910-184114.json`（`race_clean=true`、`real_turn{user_rows:1,assistant_rows:1}`、`design_notice=[]`） | 新增 `application/core/agentteam`（`TeamSpec`/`RoleSpec`、`Normalize` 顺序校验、`Factory.Materialize` 幂等装配、`Registry` CRUD、preset = goal-a2a / review-team / research-team）、`sessionstore/team_registry.go`（`session/team/roles.json` 整份替换型 + `EnsureRoleSessionWorkspace` 幂等 + `ReadLifecycleOrderWorkspace`）、`application/contract/dto/agentteam.go`（纯 DTO）、`application/core/agentteam_service.go`（窄转发）、`internal/adapters/agentteam_ports.go`（DTO↔存储映射）、`gui/headless_team.go`（`team.*` 6 方法）。**AT8 证据**：同一工厂实例化 goal 之外的 review-team（`TestSecondTeamThroughSameFactory` + 真实 API 同进程装配两个团队）。**设计偏差观察（真实 API）**：① 首轮冒烟 provider 调用失败仍会"通过"——已在探针加 `real_turn` 断言（assistant 行必须落地）与 `DATA RACE` 检查，防止无模型轮次冒充通过；② 只装配顺序、从未 sync 时 `role.snapshot` 报 `message head 缺 floor`（属预期：floor 由 sequencer sync 发布）——探针补 draft→sync 后 floor 正常随 head 发布（`floor{role_name:tl,role_session_id:goal-a2a-tl}`）且 `DesignWarnings=[]`。**pprof 结论**：mutex/block profile 中无 role/agentteam/team_registry 帧；mutex 头部为 pprof hook 日志与 `context_runtime.PrepareExecutionContext`，block 主要为 actor/stream 等待，未见死锁环与新链路热点 |
...
| **S27** | R2/R4 端口/DTO 收口：把当前 core 内部 `roleSessionPort` 与 headless 的 sessionstore 透传形状迁到 `application/contract` 纯 DTO（`RoleSessionPort`/`RoleDraftPort`/`SequencerPort`/`SchedulePort`），避免 GUI 直接依赖存储类型 | §8.3、应用层接线提示词 §R2/R4 | `application/contract/ports.go`、`application/contract/dto/rolesession.go`、`application/core/role_session.go`、`internal/adapters/session_workspace_ports.go`、`gui/headless.go` | **已收口（2026-09-10）**：新增 `dto.RoleRow/RoleDraftRow/RoleSessionInfo/RoleDraftSyncResult/RoleFloor/CompactFrameRef/RoleSnapshot/RoleWireSnapshot/ScheduleEventPayload` 与 `contract.RoleSessionPort`/`contract.SchedulePort`（可选能力，类型断言发现）；`application/core` 与 `gui/headless` 不再 import sessionstore，映射职责集中在 `internal/adapters.SessionPort`。边界回归 `e2e/dto_boundary_test.go`（禁止 import 泄漏 + JSON 字段名锚定）。headless `role.*`/`schedule.*` 的 JSON 形状不变，现有单测与真实 API 冒烟口径不受影响 |
