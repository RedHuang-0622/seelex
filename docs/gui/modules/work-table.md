...
6. 子代理详情弹窗实时分类（2026-09-07）：打开详情后
   `SubagentSessionDetail` 返回按 tab 分类的数据面（会话记录/第一视角阶段
   历史/上下文/功能打点 trace/事件时间线/工具活动/输出，来源含
   SubAgentTree 与工作台行兜底）；前端低频权威刷新 + `seelex:subagent_live`
   即时行双通道，各 tab 独立更新，不再只在打开瞬间取一次快照。
