...
| 测试/平台 | 有条件通过 | B+ | 本地逻辑、构建和静态视觉通过；race/govulncheck/多平台 runner 等待推送后远端结论 |
...
| GUI-CI-001 gui 分支触发 | 通过 | `.github/workflows/ci.yml:5-10` |
| GUI-CI-002 GUI tests job | 通过 | `.github/workflows/ci.yml:83-110`；本地等价命令全过 |
...
3. 本机 `CGO_ENABLED=0`，race 和 GitHub Action 漏洞扫描必须以远端为准。

### 建议

1. 后续加入 fake Bridge + Playwright，并为 Max/Lite 建立视觉回归基线。
2. 将 `GUI tests` 设为 `gui` 分支必需检查。
3. 远端首次全绿后在发布记录中保存 run URL。

## 6. 最终判断

本地审查通过，可以提交并推送。是否达到发布级“全通过”，以推送后的 GitHub Actions 四组门禁为最终依据。
