...
func (m *Merger) MergeBack(parent, child *snapshot.ContextSnapshot) error {
	if parent == nil { return fmt.Errorf("merger: nil parent") }
	if child == nil { return fmt.Errorf("merger: nil child") }
	...
	// replace
	if child.Progress != "" { parent.Progress = child.Progress }
	if len(child.PendingWork) > 0 { parent.PendingWork = copyStrings(child.PendingWork) }

	// append + dedup
	for _, cc := range child.Constraints {
		if !slices.Contains(parent.Constraints, cc) { parent.Constraints = append(parent.Constraints, cc) }
	}
	...
	// escape replace
	if child.Escape != nil { parent.Escape = child.Escape }
	return nil
}
