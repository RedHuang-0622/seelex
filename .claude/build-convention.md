...
约束：
- `dist/` 根除 P1~P4 外不得出现任何文件/目录（含 zip、版本目录、游离 exe）。
...
| 分区 | 固定文件夹 | 内容 | 写入方 |
|------|-----------|------|--------|
| T1 GUI 暂存区 | `tmp/build/stage-gui/` | `seelex-gui.exe` + `version.txt` | flow `Stage` |
| T2 冒烟报告 | `tmp/build/smoke/` | `smoke-*.log` / `boot-*.log` / `store-*` | flow `Smoke` |
| T3 回滚 stash | `tmp/build/stash/seelex-gui-dev/` | `seelex-gui.previous.exe` + 最近 5 份历史 + `README.txt` | flow `Deploy`/`Rollback` |
| T4 部署日志 | `tmp/build/deploy.log` | 追加的 deploy/rollback 记录 | flow `Deploy` |

约束：流程脚本（`seelex-flow.ps1`）只写上述 T1~T4；`tmp/` 其余目录是个人
草稿区，不属构建管线，规范不管理它们。
...
| dev GUI 分阶段流程 | `make dev-flow VERSION=vX.Y.Z`（或 `scripts/seelex-flow.ps1`） | Stage→`tmp/build/stage-gui`，Deploy→`dist/seelex-gui-dev/`，Smoke→`tmp/build/smoke`，Rollback→`tmp/build/stash/...`，Release→`dist/<os>-<arch>/`+`dist/archive/` |
| 提交后快速重建 dev | post-commit hook（`build-dev.sh`） | CLI→`dist/dev/seelex.exe`，GUI→`dist/seelex-gui-dev/seelex-gui.exe` |
| 全量清理（保留 P2） | `make clean`（`CLEAN_DEV=1` 才删 P2） | P1/P3/P4 |
...
3. `make guard-dist-layout` 校验 `dist/` 根只有 P1~P4，多余条目直接报错，
   防止任何人再往根里放东西。
