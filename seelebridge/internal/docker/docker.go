// Package docker 承载 Docker 守护进程自动恢复（2026-08-07 根治）：
// 真实环境有 docker CLI 但 Docker Desktop 守护进程未运行是常见状态——bash
// 工具直接报原始 pipe 错误对模型毫无帮助。现在：命令失败且错误匹配
// daemon-down 模式 → 自动启动 Docker Desktop → 轮询就绪 → 由调用方重跑一次
// 命令。域内不依赖 seelebridge 根包。
package docker
...
// desktopExe 是 Docker Desktop 的固定安装路径（回退启动路径）。
const desktopExe = `C:\Program Files\Docker\Docker\Docker Desktop.exe`

// daemonDownMarkers 是守护进程未运行 的典型错误模式
// （Windows 命名管道不存在/连接失败），命中即触发自动恢复。
var daemonDownMarkers = []string{
...
// IsDaemonDown 判断 bash 命令的失败输出是否指向 docker 守护进程
// 未运行（CLI 存在但 daemon 不在）。输出为空 → false（无法判定不触发）。
func IsDaemonDown(stdout, stderr string) bool {
	haystack := strings.ToLower(stdout + "\n" + stderr)
	for _, marker := range daemonDownMarkers {
...
// CLILookup 是 docker CLI 的 PATH 探测（测试可覆盖的注入缝；并行测试禁止）。
var CLILookup = exec.LookPath

// fixedPaths 是 Docker Desktop 的固定安装路径回退（测试可覆盖）。
var fixedPaths = []string{
	`C:\Program Files\Docker\Docker\resources\bin\docker.exe`,
	`C:\Program Files\Docker\Docker\resources\bin\docker`,
}

// CLIPath 定位 docker CLI：PATH 优先，回退 Docker Desktop 固定路径。
// 找不到 → ""（环境无 docker，不触发自动恢复）。
func CLIPath() string {
	if path, err := CLILookup("docker"); err == nil {
		return path
	}
	for _, candidate := range fixedPaths {
...
// Prober 抽象守护进程探测/启动（测试注入点；生产用真实实现）。
type Prober struct {
...
// RealProber 是生产探针：docker info 快速探测 + Windows 启动路径。
func RealProber() Prober {
	return Prober{
		Up: func(ctx context.Context) bool {
			...
			defer cancel()
			cmd := exec.CommandContext(probeCtx, CLIPath(), "info", "--format", "{{.ServerVersion}}")
		...
		Start: func(ctx context.Context) error {
			...
			if !security.FileExists(desktopExe) {
				return fmt.Errorf("docker: Docker Desktop is not installed at %s", desktopExe)
			}
			cmd := exec.CommandContext(ctx, desktopExe)
			security.ConfigureHiddenCommand(cmd)
			return cmd.Start()
		},
	}
...
// EnsureDaemon 确保守护进程就绪：未就绪 → Start → 轮询 Up 直到
// waitTimeout。已就绪直接返回（不重复启动）。返回错误时附带启动输出说明。
func EnsureDaemon(ctx context.Context, prober Prober, waitTimeout time.Duration) error {
...
// EnsureForRuntime 是 Runtime 的接线面：按 limits 配置执行自动恢复
// （disable_docker_auto_start 关闭时返回 nil 表示"不处理"）。
// probe 已注入（测试）时使用注入探针，否则用真实实现。
func EnsureForRuntime(ctx context.Context, disableAutoStart bool, startTimeoutSec int, probe Prober) error {
	if disableAutoStart {
		return nil
	}
	timeout := time.Duration(startTimeoutSec) * time.Second
	if timeout <= 0 {
	...
	return EnsureDaemon(ctx, prober, timeout)
}

// Hint 生成模型可读的恢复说明（重跑仍失败时附加）。
func Hint(startErr error) string {
