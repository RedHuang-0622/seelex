...
The adapter accepts node entries with `id` or `key`, edge entries with `from`/`source` and `to`/`target`, and adjacency targets written as `{ "to": "id" }`. Canonical node specs belong under `nodes`; as a narrowly scoped recovery compatibility, a referenced top-level node spec containing only `input` and optional `kind` is merged into `nodes` before validation. It never guesses a missing edge source or target, and rejects unrelated top-level fields such as `item`; invalid references return a pre-execution `plan_load` error and can use the bounded corrective retry path.
