...
func TestEnginePortLazyResumeCreatesOnlyRequestedSession(t *testing.T) {
	...
	if err := port.ReplaceHistory("session-resume", []contract.EngineMessage{{Role: "user", Content: "restored", ContentSet: true}}); err != nil {
		t.Fatal(err)
	}
...
func TestDecodeSessionRecordPreservesV2Components(t *testing.T) {
	want := model.SessionRecord{Version: 2, ID: "session-b", Title: model.SessionTitle{Value: "Stable title", Source: "user"}}
